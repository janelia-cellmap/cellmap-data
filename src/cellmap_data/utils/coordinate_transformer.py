...
class CoordinateTransformer:
    ...
    def __init__(self, axis_order: str = "zyx", enable_caching: bool = True):
        ...
        logger.debug(f"CoordinateTransformer initialized with axis_order='{axis_order}', caching={enable_caching}")
    ...
    def apply_spatial_transforms(
        ...
        try:
            ...
            if spatial_transforms is None:
                ...
                transformed_center = self._apply_transform_operations(center, spatial_transforms)
            ...
            self.transform_stats["average_transform_time"] = (
                self.transform_stats["total_transform_time"] / self.transform_stats["total_transforms"]
            )
    ...
    def _apply_transform_operations(
        self,
        center: Dict[str, float],
        spatial_transforms: Mapping[str, Any]
    ) -> Dict[str, float]:
        ...
        if "rotation" in spatial_transforms:
            transformed_center = self._apply_rotation(transformed_center, spatial_transforms["rotation"])
            
        # Apply translation transforms
        if "translation" in spatial_transforms:
            transformed_center = self._apply_translation(transformed_center, spatial_transforms["translation"])
            
        # Apply scaling transforms
        if "scaling" in spatial_transforms:
            transformed_center = self._apply_scaling(transformed_center, spatial_transforms["scaling"])
    ...
    def _apply_rotation(
        self,
        center: Dict[str, float],
        rotation_config: Mapping[str, Any]
    ) -> Dict[str, float]:
    ...
    def apply_cached_transforms(
        ...
        if not self.enable_caching or self._coordinate_cache is None:
            ...
            transformed_center = self.apply_spatial_transforms(center, spatial_transforms)
            return transformed_center, False
            
        # Check cache first
        reference_frame_id = self.get_reference_frame_id(center)
        
        cached_result = self._coordinate_cache.get_cached_transforms(reference_frame_id, spatial_transforms)
    ...
    def compute_bounding_box(
        ...
        """
        if not centers:
            raise CoordinateTransformError("Cannot compute bounding box for empty centers list")
        ...
        for axis in self.axis_order:
            ...
            axis_coords = [center.get(axis, 0.0) for center in centers if axis in center]
