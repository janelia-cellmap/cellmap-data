...
class DatasetValidator:
    ...
    def validate_array_config(
        ...
        if not arrays:
            logger.debug(
                f"Empty {array_type} arrays configuration - skipping validation"
            )
            return
        ...
        logger.debug(
            f"Array configuration validation passed for {len(arrays)} {array_type} arrays"
        )
    ...
    def validate_performance_config(
        ...
        if max_workers is not None:
            ...
            if max_workers > 64:  # Reasonable upper limit
                logger.warning(
                    f"max_workers={max_workers} is very high, consider reducing"
                )
        ...
        if not all(c in valid_axes for c in axis_order.lower()):
            logger.warning(
                f"axis_order '{axis_order}' contains non-standard axis names"
            )
    ...
    def validate_dataset_config(
        ...
        self.validate_transforms(
            spatial_transforms, raw_value_transforms, target_value_transforms
        )
