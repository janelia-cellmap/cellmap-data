...
class CoordinateTransformer:
    ...
    def generate_spatial_transforms(
        self, 
        spatial_transform_config: Optional[Mapping[str, Any]] = None
        ...
        try:
            for transform, params in spatial_transform_config.items():
                if transform == "mirror":
                    spatial_transforms[transform] = self._generate_mirror_transform(params)
                elif transform == "transpose":
                    spatial_transforms[transform] = self._generate_transpose_transform(params)
                elif transform == "rotate":
                    spatial_transforms[transform] = self._generate_rotate_transform(params)
                else:
    ...
    def _generate_transpose_transform(self, params: Mapping[str, Any]) -> Dict[str, int]:
    ...
    def convert_center_to_index(
        ...
        try:
            ...
            coords = tuple(
                int(center[axis]) for axis in self.axis_order
            )
    ...
    def calculate_bounding_box(
        self, 
        array_configs: Mapping[str, Mapping[str, Sequence[Union[int, float]]]]
    ...
    def apply_cached_transforms(
        ...
        try:
            ...
            if cached_entry is not None:
                ...
                logger.debug(f"Applied cached transforms for frame {reference_frame_id}")
                return True
            else:
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        """
        total_requests = self._transform_stats["cache_hits"] + self._transform_stats["cache_misses"]
        cache_hit_rate = (
    ...
    def __str__(self) -> str:
        ...
        return (f"CoordinateTransformer(axis_order='{self.axis_order}', "
