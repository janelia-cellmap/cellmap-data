...
class TestDatasetWriterGPUTransfer:
    ...
    def test_collate_fn_gpu_transfer(self):
        ...
        mock_batch = [
            {
                "input_array": torch.randn(1, 8, 8, 8),
                "idx": torch.tensor(0)
            },
            {
                "input_array": torch.randn(1, 8, 8, 8), 
        ...
        for batch_item in mock_batch:
            for key, tensor in batch_item.items():
                assert tensor.device.type == "cpu", f"Input tensor {key} should be on CPU"
        ...
        for key, tensor in result.items():
            assert tensor.device.type == "cuda", f"Output tensor {key} should be on CUDA device, got {tensor.device}"
    ...
    def test_loader_uses_gpu_transfer(self):
        ...
        with patch('cellmap_data.dataset_writer.CellMapImage'), \
            ...
            writer = CellMapDatasetWriter(
                ...
                classes=["test_class"],
                input_arrays={"test_input": {"shape": [8, 8, 8], "scale": [1.0, 1.0, 1.0]}},
                target_arrays={"test_target": {"shape": [4, 4, 4], "scale": [2.0, 2.0, 2.0]}},
                target_bounds={"test_target": {"x": [0.0, 8.0], "y": [0.0, 8.0], "z": [0.0, 8.0]}},
                device="cuda"
            )
            ...
            # Verify loader has correct device
            assert str(loader.device) == "cuda"
            ...
            for key, tensor in result.items():
                assert tensor.device.type == "cuda", f"Loader output tensor {key} should be on CUDA device"
